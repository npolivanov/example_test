...
export default class AddObjects extends React.Component {
    ...
    componentDidMount() {
        ...
        const _this = this;
        axios.post( "http://poliva0s.beget.tech/serv.php", data)
    ...
    addObjects () {
        ...
        axios.post( "http://poliva0s.beget.tech/addObj.php", data) 
        .then(function (response) {
        ...
        this.props.addObjects( this.state.item, this.item_id );
    ...
    render () {
        const status_item = this.status.map( (item, i) => {
            if ( item === this.state.item.status) {
                    return <option selected key={i}>{item}</option>;
            }else {
            ...
            return (
                <li>
                    <div className="form-check form-check-inline">
                        <input  checked={item.active} type="checkbox"  onChange={this.changeServes} />
                        <label className="form-check-label" >{item.title}  дата: {item.date}</label>
                    </div>
                 </li>
            );
        });
        return (
            <Ul className="list-group-item list-group-item-action" >
                ...
                <div className="form-group col-md-4">
                        <select id="inputState" className="form-control" onChange={this.selectType}>
                                {type_item }
                        </select>
                    </div>
                    <input value={this.state.item.date_include} onChange={this.inputAfter} className="form-control"/>
                    <input value={this.state.item.date_diactive} onChange={this.inputBefore} className="form-control"/>
                    <div className="form-group col-md-4">
                        <select  className="form-control"onChange={this.selectStatus} >
                                {status_item }
                        </select>
                    </div>
                </div>
                ...
                <button className="addObject btn btn-dark" onClick={this.addObjects.bind(this)}>+ Добавить новый объект</button>
            </Ul>
        );
    };
};